...
export default defineComponent({
  ...
  this.appAxios.get(`/liste?BoardId=${this.activeBoardId}&SkipCount=0&MaxResultCount=100`).then((response) => {
    ...
    // Eğer listedeki kart sayısı 0 ise boş bir kart ekleyin
    this.listItem.items.forEach((item) => {
      if (this.isEmptyList(item)) {
        this.addEmptyCard(item.id);
      }
    });
  });
},

    isEmptyList(listItem: ListItem){
    ...
    addEmptyCard(listId) {
  const emptyCard = {
    name: '', // Kartın adını ayarlayabilirsiniz
    description: '',
    listeId: listId,
    position: 0, // Kartın pozisyonunu ayarlayabilirsiniz
    id: 0, // Eğer yeni bir kart oluşturuyorsanız, bu ID'yi sunucudan alabilirsiniz veya benzersiz bir şekilde oluşturabilirsiniz
  };

  // Boş kartı listenin kartlar listesine ekleyin
  const list = this.listItem.items.find((item) => item.id === listId);
  list.cards.push(emptyCard);

  // Sunucuya kaydetmek için API isteği gönderebilirsiniz
  // this.appAxios.post('/card', emptyCard).then((response) => {
  //   // Kart başarıyla oluşturuldu
  //   // Yeni kartın ID'sini güncelleyebilirsiniz
  //   emptyCard.id = response.data.id;
  // });
},
